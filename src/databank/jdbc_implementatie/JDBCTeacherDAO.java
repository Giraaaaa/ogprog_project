...
public class JDBCTeacherDAO extends JDBCAbstractDAO implements TeacherDAO {
    ...
    @Override
    public Iterable<Teacher> getTeachers() throws SQLException {
        try (PreparedStatement stmnt = prepare("SELECT * FROM teacher")) {
            ...
            throw new SQLException("Could not retrieve teachers from database.");
        }
    }

    public Teacher findByName(String naam) throws SQLException {
        try (PreparedStatement stmnt = prepare("SELECT * FROM teacher WHERE name = ?")) {
            ...
            try (ResultSet set = stmnt.executeQuery()) {
                ...
                throw new SQLException("Could not retrieve teacher from database.");
            }
        }
    }

    public void updateTeacher(String name, int id) throws SQLException {
        try (PreparedStatement stmnt = prepare("UPDATE teacher SET name = ? WHERE id = ?")) {
            stmnt.setString(1, name);
            stmnt.setInt(2, id);
            stmnt.executeUpdate();
        } catch (SQLException ex) {
            throw new SQLException("Could not update this teacher.");
        }
    }

    //Returns the id of the created person
    public int createTeacher(String name) throws SQLException {
        try (PreparedStatement stmnt = prepare("INSERT INTO teacher(name) VALUES (?)")) {
            ...
            throw new SQLException("Could not create teacher.");
        }
    }
}
